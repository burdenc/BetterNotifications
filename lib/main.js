...
exports.main = function(options) {
	...
	simplePrefs.on("notif_center.enabled", function () {
		if(simplePrefs.prefs['notif_center.enabled']) {
			setupUI();
...
function setupUI() {
	...
	({
		...
		onShow: function() {
			...
			this.port.emit('onOpen', {'template':template, 'notifications':ss.notifications, 'perPage': simplePrefs.prefs['notif_center.per_page']});
			if(oldUI)
...
function onNewNotification(data) {	
	...
	//console.log(data['alertListener']);
	
	//data['alertListener'].observe(null, 'alertclickcallback', data['alertCookie']);
