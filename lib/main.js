...
exports.startup = function(data, reason) {
	console.error('Starting Better Notifications');
		...
		({
			...
			/*onClick: function() {
				notifications.notify({
				  ...
				  //iconURL: "chrome://global/skin/alerts/notification-48.png"
				});
				console.log("Notifs: " + JSON.stringify(savedNotifications));
			}*/
...
function onNewWindow(chromeWindow) {
	...
	chromeWindow.onclick = function(event) {
		if(event.button == 0 && event.shiftKey == false) {
			...
			console.error('Calling the call back!');
		}
