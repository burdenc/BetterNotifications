...
function setupUI() {
	...
	({
		...
		onShow: function() {
			...
			this.port.emit('onOpen', {'template':template, 'view':{'notifications':ss.notifications.slice(0,5)}, 'style':'.x-button {width: 16px; height: 16px; background-image: url('+self.data.url('data/buttons.png')+') 0 0;}'});
			if(oldUI)
